...
outdir = "/media/brian/CCTS/SkinSegmentation"

#@require(sqlite3,subprocess)
def runSkin(cmd):
    conn = sqlite3.connect(cmd[2])
    cursor = conn.cursor()
    rslt = subprocess.call("/home/brian/anaconda/bin/vtkpython -E /home/brian/bin/dicom2stl.py -t skin -o %s %s"%(cmd[1],cmd[0]),shell=True)
...
def main():
    ...
    if not os.path.exists(outdir):
        os.mkdir(outdir)
    dbname = os.path.join(outdir,"skinProcessingResults.sqlite")
